...
set -x
pip install awscli
tar cjpf "$root"/../prefix-"$id".tar.bz2 --xattrs-include='*.*' --numeric-owner --one-file-system -C "$root" $(find "$root" -maxdepth 1 -type d -not -name dev -not -name sys -not -name proc -not -name tmp -not -name run -not -name boot | tr '\n' ' ')
