...
run "$root" emerge repoman git
echo "travis_fold:end:repoman_install"

# run repoman on codebase
echo "travis_fold:start:repoman_run"
run "$root" cd /usr/local/portage '&&' repoman -v full
echo "travis_fold:end:repoman_run"

# merge repoman
echo "travis_fold:start:installing_packages"
run "$root" emerge $packages
echo "travis_fold:end:installing_packages"
