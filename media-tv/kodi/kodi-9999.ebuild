...
IUSE="agile airplay alsa bluetooth bluray caps cec +css dbus debug dvd gles libressl libusb lirc mysql nfs nonfree +opengl +ssl pulseaudio samba sftp systemd +system-ffmpeg test +udev udisks upnp upower vaapi vdpau webserver +X +xslt zeroconf"
REQUIRED_USE="
	${PYTHON_REQUIRED_USE}
	|| ( gles opengl )
	udev? ( !libusb )
...
*)
	...
	MY_P="${PN}-${MY_PV}"
	SRC_URI+="
		!agile? ( https://github.com/xbmc/xbmc/archive/${MY_PV}-${CODENAME}.tar.gz -> ${MY_P}.tar.gz )
		agile? ( https://github.com/FernetMenta/kodi-agile/archive/${MY_PV}-${CODENAME}.tar.gz -> ${MY_P}.tar.gz )
...
src_prepare() {
	...
	if use agile && use !vdpau; then
		eapply "${FILESDIR}"/${P}-no-vdpau.patch
	fi
